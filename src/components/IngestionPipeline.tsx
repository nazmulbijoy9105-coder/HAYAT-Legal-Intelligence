...
import { Upload, FileText, CheckCircle, RotateCcw, AlertTriangle, AlertCircle, ShieldCheck, Cpu, Sliders, Image, Sparkles } from 'lucide-react';
import { IngestionMetrics, OCRBoundingBox } from '../types';
import { DocIntelLayers } from './DocIntelLayers';

export function IngestionPipeline() {
  const [activeSample, setActiveSample] = useState<'gazette' | 'judgment' | 'custom' | 'turag' | null>(null);
  ...
  const processFile = async (file: File) => {
    ...
    // 1. Client-side File Type Validation (any PDF allowed)
    const isPDF = file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf');
    if (!isPDF) {
      setFileValidationError(`Validation Error: Ingestion sandbox currently only accepts Portable Document Format (PDF) files. Detected: "${file.name}" of type "${file.type || 'unknown'}".`);
    ...
    if (file.size === 0) {
      setFileValidationError(`Integrity Error: The uploaded document "${file.name}" is corrupted or empty (0 bytes).`);
      setIsProcessing(false);
      return;
    }
    ...
    if (file.size > maxLimit) {
      setFileValidationError(`Integrity Error: File size (${(file.size / (1024 * 1024)).toFixed(2)} MB) exceeds the 25MB administrative threshold.`);
      setIsProcessing(false);
      return;
    }

    // 3. Compute real SHA-256 for integrity check
    let sha256Hash = '';
    try {
      ...
      console.error("SHA-256 checksum computation failed:", e);
    ...
    try {
      const res = await fetch('/api/ingest', {
        ...
        body: JSON.stringify({ sampleType: 'custom', fileName: file.name })
      });
      if (!res.ok) {
      ...
      // Override server mocks with the actual uploaded PDF's calculated metadata!
      const finalMetrics = {
        ...data.metrics,
        fileSize: humanSize,
        sha256: sha256Hash,
  ...
  const triggerIngest = async (sampleType: 'gazette' | 'judgment' | 'custom' | 'turag') => {
    setIsProcessing(true);
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6" id="ingestion-section">
        ...
        <div className="xl:col-span-5 space-y-6 text-left">
          ...
          {/* Drag-and-Drop Dropzone with File Selection */}
          <div
            onDragOver={handleDragOver}
            onDragLeave={handleDragLeave}
            onDrop={handleDrop}
            onClick={onDropzoneClick}
            className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all duration-200 relative group ${
              dragOver
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center space-y-2">
              <Upload className="w-8 h-8 text-slate-500 group-hover:text-emerald-400 transition-colors animate-bounce" />
              <span className="font-sans font-medium text-xs text-slate-200">
                Drag & Drop PDF document here or click to browse
              </span>
              <span className="text-[10px] text-slate-400 font-mono">
                Supports any PDF document up to 25MB • Real-time SHA256 integrity check
              </span>
            </div>
          </div>
            ...
            <div className="bg-red-500/10 p-4 rounded-xl border border-red-500/20 text-left flex items-start gap-2.5">
              ...
              <div>
                <span className="text-[10px] font-mono font-bold text-red-400 block uppercase">
                  File Ingestion Validation Exception:
                </span>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              >
                ...
                <span className="text-[10px] text-slate-400 mt-0.5 block">Dowry Act 2018</span>
              </button>
              ...
              >
                <FileText className="w-4 h-4 text-sky-400 mb-1.5" />
                <span className="font-semibold text-slate-200 block">Supreme Court Judgment</span>
                <span className="text-[10px] text-slate-400 mt-0.5 block">State v. Opu Case</span>
              </button>

              <button
                onClick={() => triggerIngest('turag')}
                disabled={isProcessing}
                className={`p-3 rounded-xl border text-left transition-all text-xs disabled:opacity-50 cursor-pointer ${
                  activeSample === 'turag'
                    ? 'border-emerald-500/55 bg-emerald-500/10'
              ...
              >
                ...
                <span className="font-semibold text-slate-200 block">Turag River Case</span>
                <span className="text-[10px] text-slate-400 mt-0.5 block">Civil Appeal 3039/2019</span>
              </button>
            </div>
          </div>
      ...
      {metrics && (
        <DocIntelLayers sampleType={activeSample} fileName={fileName} metrics={metrics} />
      )}
    </div>
  );
}
