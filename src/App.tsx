  ...
  const handleTriggerDecomposition = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/analyze-document', {
        ...
        body: JSON.stringify({ text: customText, fileName: customFileName || 'manual_upload.txt' })
      });
      ...
      if (res.ok) {
        ...
        newDecomposed = {
          ...
          court: ext.courtOrAuthority || 'Supreme Court of Bangladesh',
          country: ext.courtOrAuthority?.toLowerCase().includes('pakistan') ? 'Pakistan' : 'Bangladesh',
          bench: ext.judgesOrOfficers || ['Presiding Judges Bench'],
          ...
          proceduralHistory: [
            ...
            { id: 'ph2', stage: 'High Court', outcome: ext.summary?.substring(0, 100) + '...', arrow: true },
            { id: 'ph3', stage: 'Supreme Court', outcome: 'Appeal Allowed. High fidelity judgment delivered.' }
          ],
          facts: (ext.summary || '').split('. ').filter((s: string) => s.trim().length > 5).map((s: string, idx: number) => ({
            id: `dfact-${idx}`,
            text: s.trim() + (s.endsWith('.') ? '' : '.')
          })),
          issues: (ext.ilrmf?.issues || []).map((is: string, idx: number) => ({
            id: `dissue-${idx}`,
            text: is,
            answer: 'Yes',
            elaborated: ext.ilrmf?.application || 'Elements and thresholds satisfied completely under procedural rules.'
          })),
          statutes: (ext.actsCited || []).map((act: any, idx: number) => ({
            id: `dstat-${idx}`,
            actName: act.actName,
            section: act.sections?.join(', ') || 'General',
            role: `Primary statutory authority supporting the ratio. Temporal status is ${act.temporalStatus}.`,
            temporalStatus: act.temporalStatus || 'Valid'
          })),
          principles: (ext.highlights || []).filter((h: any) => h.category.toLowerCase().includes('ratio') || h.category.toLowerCase().includes('statute')).map((h: any, idx: number) => ({
            id: `dprinc-${idx}`,
            text: h.text
          })),
          ratioDecidendi: (ext.highlights || []).filter((h: any) => h.category.toLowerCase().includes('ruling') || h.category.toLowerCase().includes('decidendi')).map((h: any, idx: number) => ({
            id: `dratio-${idx}`,
            text: h.text
          })),
          directions: [
          ...
          paragraphs: [
            { id: 'dpara1', index: 1, category: 'Metadata', text: `${ext.title || 'Dynamic Document'}. Decision Date: ${ext.date || '2026-07-14'}.` },
            { id: 'dpara2', index: 2, category: 'Facts', text: ext.summary || 'Summary details extracted from text corpus.' },
            { id: 'dpara3', index: 3, category: 'Law', text: `Primary legislation cited: ${ext.actsCited?.map((a: any) => `${a.actName} (Sections ${a.sections?.join(', ')})`).join('; ') || 'None'}.` },
            { id: 'dpara4', index: 4, category: 'Reasoning', text: ext.ilrmf?.application || 'No explicit reasoning text found.' },
            { id: 'dpara5', index: 5, category: 'Directions', text: ext.ilrmf?.conclusion || 'Final judgment conclusion issued.' }
          ],
          knowledgeGraph: {
            nodes: [
              { id: 'case-dyn', label: (ext.title || 'Dynamic Case').substring(0, 25), type: 'case' },
              ...(ext.actsCited || []).map((a: any, idx: number) => ({ id: `stat-dyn-${idx}`, label: a.actName, type: 'statute' as const })),
              ...(ext.judgesOrOfficers || []).map((j: any, idx: number) => ({ id: `judge-dyn-${idx}`, label: j, type: 'judge' as const }))
            ],
            links: [
              ...(ext.actsCited || []).map((a: any, idx: number) => ({ source: 'case-dyn', target: `stat-dyn-${idx}`, label: 'references' })),
              ...(ext.judgesOrOfficers || []).map((j: any, idx: number) => ({ source: 'case-dyn', target: `judge-dyn-${idx}`, label: 'presided by' }))
            ]
          },
          aiSummary: {
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-semibold px-4 py-2.5 rounded-xl flex items-center gap-2 transition-all shadow-[0_4px_20px_rgba(16,185,129,0.15)] cursor-pointer"
            >
